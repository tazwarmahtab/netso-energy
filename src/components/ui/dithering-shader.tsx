...
interface DitheringShaderProps {
  /** Fixed canvas resolution in px. Omit (or set to 0) to auto-fill parent container. */
  width?: number
  height?: number
  colorBack?: string
  colorFront?: string
...
export function DitheringShader({
  ...
  // Track resolved canvas dimensions (either from props or container)
  const [resolvedW, setResolvedW] = useState(width || 800)
  const [resolvedH, setResolvedH] = useState(height || 600)

  // ResizeObserver for responsive / full-bleed mode
  useEffect(() => {
    if (width && height) return // fixed mode — skip observer
    ...
    const ro = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        if (w > 0 && h > 0) {
          setResolvedW(Math.round(w))
          setResolvedH(Math.round(h))
        }
      }
    })
  ...
  useEffect(() => {
    ...
    canvas.width = resolvedW
    canvas.height = resolvedH
    gl.viewport(0, 0, resolvedW, resolvedH)
    ...
    const render = () => {
      ...
      const loc = uniformLocationsRef.current
      if (loc.u_time) context.uniform1f(loc.u_time, currentTime)
      if (loc.u_resolution) context.uniform2f(loc.u_resolution, resolvedW, resolvedH)
      if (loc.u_colorBack) context.uniform4fv(loc.u_colorBack, hexToRgba(colorBack))
      if (loc.u_colorFront) context.uniform4fv(loc.u_colorFront, hexToRgba(colorFront))
      if (loc.u_shape) context.uniform1f(loc.u_shape, DitheringShapes[shape])
      if (loc.u_type) context.uniform1f(loc.u_type, DitheringTypes[type])
      if (loc.u_pxSize) context.uniform1f(loc.u_pxSize, pxSize)
  ...
  }, [resolvedW, resolvedH, colorBack, colorFront, shape, type, pxSize, speed])

  const isFixedMode = width > 0 && height > 0
