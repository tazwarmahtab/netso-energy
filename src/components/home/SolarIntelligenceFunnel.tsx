...
const facilityOptions: Array<{ value: FacilityType; label: string; icon: typeof Factory }> = [
  { value: "factory", label: "Factory", icon: Factory }, { value: "commercial", label: "Commercial building", icon: Building2 },
  { value: "school", label: "School / institution", icon: GraduationCap }, { value: "hospital", label: "Hospital", icon: HeartPulse },
  { value: "hotel", label: "Hotel", icon: Hotel }, { value: "warehouse", label: "Warehouse", icon: Warehouse },
];
...
export function SolarIntelligenceFunnel() {
  ...
  const [submitted, setSubmitted] = useState<StartAssessmentSessionResponse["session"] | null>(null);
  ...
  const goNext = () => { setError(""); setStep((value) => Math.min(value + 1, steps.length - 1)); };
  const goBack = () => { setError(""); setStep((value) => Math.max(value - 1, 0)); };
  ...
  const submit = async () => {
    ...
    const estimate = { systemKwp: result.recommendedCapacityKwp, systemKwpRange: { low: result.recommendedCapacityKwp * 0.9, midpoint: result.recommendedCapacityKwp, high: result.recommendedCapacityKwp * 1.1 }, monthlySavingsBdt: (result.annualSavingsBdt ?? 0) / 12, monthlySavingsBdtRange: { low: Math.max(0, (result.annualSavingsBdt ?? 0) * 0.9 / 12), midpoint: (result.annualSavingsBdt ?? 0) / 12, high: (result.annualSavingsBdt ?? 0) * 1.1 / 12 }, annualSavingsBdt: result.annualSavingsBdt ?? 0, ppaTermSavingsBdt: result.twentyYearSavingsBdt ?? 0, ppaTermSavingsBdtRange: { low: (result.twentyYearSavingsBdt ?? 0) * 0.9, midpoint: result.twentyYearSavingsBdt ?? 0, high: (result.twentyYearSavingsBdt ?? 0) * 1.1 }, annualGenerationKwh: result.annualGenerationKwh, co2SavedTonnes: result.co2AvoidedTonnes, confidenceLabel: "resco_ppa" as const, assumptions: result.flags, disclaimer: result.disclaimer, assumptionsVersion: result.assumptionsVersion, confidence: result.confidence };
    try {
      if (!isSupabaseBrowserConfigured()) throw new AssessmentBackendUnavailableError("Assessment backend unavailable");
      const payload = createCalculatorLeadPayload({ preferredLanguage: language, name: form.name, phone: form.phone }, { calculatorBillEstimate: num(form.monthlyBill), calculatorAreaEstimate: num(form.roofM2) ? Number(form.roofM2) / 0.092903 : undefined, modelOutput: estimate });
      const response = await startAssessmentSession(payload);
      setSubmitted(response.session);
      if (isWhatsAppConfigured() && typeof window !== "undefined") window.location.assign(buildWhatsAppStartUrl({ language, source: "calculator_handoff", sessionId: response.session.id }));
      else toast.success(bn ? "অ্যাসেসমেন্ট শুরু হয়েছে।" : "Assessment started.");
    } catch (cause) {
      if (cause instanceof AssessmentBackendUnavailableError && isWhatsAppConfigured() && typeof window !== "undefined") window.location.assign(buildWhatsAppStartUrl({ language, source: "calculator_handoff_manual" }));
      else setError(cause instanceof Error ? cause.message : (bn ? "আবার চেষ্টা করুন।" : "Please try again."));
  ...
  const headings: Record<Step, string> = { facility: bn ? "আপনার প্রতিষ্ঠান কী ধরনের?" : "What type of facility do you have?", location: bn ? "আপনার facility কোথায়?" : "Where is your facility?", energy: bn ? "আপনার বিদ্যুৎ ব্যবহার কত?" : "How much electricity does your facility use?", roof: bn ? "কতটুকু usable roof space আছে?" : "How much usable roof space do you have?", objective: bn ? "আপনার প্রধান লক্ষ্য কী?" : "What matters most to you?", results: bn ? "আপনার solar potential" : "Your solar potential", contact: bn ? "আপনার Netso assessment নিন" : "Get your Netso assessment" };
  const progress = ((step + 1) / steps.length) * 100;

  return <div className="ivory-panel mx-auto w-full max-w-4xl overflow-hidden rounded-[32px] border border-border/70 shadow-[0_24px_70px_rgba(37,23,8,0.08)]">
    <div className="px-6 pt-6 sm:px-10 sm:pt-8"><div className="flex justify-between text-xs font-semibold text-muted-foreground"><span>NETSO SOLAR INTELLIGENCE</span><span>{step + 1} / {steps.length}</span></div><div className="mt-3 h-1.5 rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${progress}%` }} /></div></div>
    <div className="p-6 sm:p-10"><h2 className="max-w-2xl font-display text-3xl tracking-[-0.03em] sm:text-5xl">{headings[current]}</h2><div className="mt-8">
