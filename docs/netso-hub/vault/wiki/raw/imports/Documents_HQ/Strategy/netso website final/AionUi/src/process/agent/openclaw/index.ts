...
export class OpenClawAgent {
  ...
  async start(): Promise<void> {
    try {
      ...
      const detectedTls = getGatewayTls();
      const isTlsEnabled = gatewayConfig.tls?.enabled ?? detectedTls?.enabled ?? false;
      const protocol = isTlsEnabled ? 'wss' : 'ws';
      const rejectUnauthorized = gatewayConfig.tls?.rejectUnauthorized ?? detectedTls?.rejectUnauthorized ?? false;
